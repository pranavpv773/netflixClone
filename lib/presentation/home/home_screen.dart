                ...
                children: [
                  BlocBuilder<HomeBloc, HomeState>(
                    builder: (context, state) {
                      ...
                      final overViewPast = state.pastYearMovieList.map((m) {
                        return "${m.overview}";
                      }).toList();

                      final overViewTenseDramaMovieList =
                          state.tenseDramaMovieList.map((m) {
                        return "${m.overview}";
                      }).toList();
                      final overViewSouthIndianMovieList =
                          state.southIndianMovieList.map((m) {
                        return "${m.overview}";
                      }).toList();
                      final overViewTrendingTvList =
                          state.trendingTvList.map((m) {
                        return "${m.overview}";
                      }).toList();
                      final overViewTrendingMovie =
                          state.trendingMovieList.map((m) {
                        return "${m.overview}";
                      }).toList();
                      final overViewPastTitle =
                          state.pastYearMovieList.map((m) {
                        return "${m.title}";
                      }).toList();

                      final titleTenseDramaMovieList =
                          state.tenseDramaMovieList.map((m) {
                        return "${m.originalTitle}";
                      }).toList();
                      final titleSouthIndianMovieList =
                          state.southIndianMovieList.map((m) {
                        return "${m.originalTitle}";
                      }).toList();
                      ...
                      return ListView(
                        children: [
                          ...
                          HomeMovieCards(
                            ...
                            overview: overViewTrendingMovie,
                            titleList: titleTrendingMovie,
                          ),
                          kheight30,
                          HomeMovieCards(
